...
export default function Leve1({ setActiveGameLevel, activeGameLevel }) {
  const { auth } = useAuth();
  const [counter, setCounter] = useState(0);
  const [timeCounter, setTimeCounter] = useState(level2Rubbishes.length + 2);
  ...
  useEffect(() => {
    if (timeCounter <= 0 && isPlaying) {
      ...
      setTimeCounter(level2Rubbishes.length + 2);
      setCounter(0);
      setIsPlaying(false);
      setOpenModal(true);
    }
